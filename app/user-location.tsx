import { useGetUserLocationQuery, useUpdateUserLocationMutation } from '@/service/profile.service';
import {
    autocompletePlaces,
    PlacePrediction,
    ResolvedPlace,
...
import { ActivityIndicator, Alert, Keyboard, Modal, ScrollView, Text, TextInput, View } from 'react-native';
...
export default function UserLocationScreen() {
    ...
    const { data: currentData, isLoading: currentLoading } = useGetUserLocationQuery(undefined);
    const [updateLocation, { isLoading: saving }] = useUpdateUserLocationMutation();
    const currentLocation = currentData && typeof currentData === 'object' && 'data' in currentData
        ? currentData.data
        : currentData ?? null;
    ...
    const commit = useCallback(
        async (place: ResolvedPlace) => {
            try {
                ...
                Alert.alert('Hata', 'Konum güncellenemedi, lütfen tekrar deneyin.');
            }
        },
    ...
    return (
        <SafeAreaView className="flex-1 bg-primary" edges={['top']}>
            <View className="bg-primary px-5 pt-4 pb-5 flex-row items-center">
                ...
                <Text className="text-white text-lg font-bold flex-1 text-center">Konum</Text>
                <Tappable
                    onPress={openMap}
            ...
            <View className="flex-1 bg-white">
                {currentLoading ? (
                    ...
                    <View className="mx-5 mt-4 p-4 bg-orange-50 rounded-2xl flex-row items-start">
                        ...
                        <View className="flex-1">
                            <Text className="text-[10px] text-primary font-bold uppercase mb-0.5">Mevcut konumunuz</Text>
                            <Text className="text-gray-800 text-[13px]" numberOfLines={2}>
                    ...
                    <View className="mx-5 mt-4 p-4 bg-gray-50 rounded-2xl">
                        <Text className="text-gray-400 text-[13px]">Henüz bir konum kaydetmediniz.</Text>
                    </View>
                )}
                ...
                <ScrollView className="flex-1 px-5 pt-2" keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
                    ...
                    >
                        ...
                        <Text className="text-gray-800 text-[14px]">Mevcut GPS konumumu kullan</Text>
                    </Tappable>
